...
#   'hover'    - take off and just hover in place for HOVER_S seconds (debug)
#   'baby'     - takeoff -> forward 3 m -> 360 turn -> hover -> 180 turn ->
#                forward 3 m back to takeoff -> land (position setpoints / go_to)
...
HOVER_S = 15.0       # 'hover' sequence: seconds to hold a stationary hover
BABY_FWD = 3.0       # 'baby' sequence: forward distance (m) out and back
BABY_YAW_S = 4.0     # 'baby' sequence: seconds per 180 deg rotation step
...
def run_hover_sequence(cf):
    """
    Take off and hold a stationary hover for HOVER_S seconds, then land. No
    horizontal motion is commanded -- the whole flight is a clean
    commanded-velocity=0 window, so any drift in mocap position/velocity during
    the hover IS the hover bias (tilt + flow bias) for that flight.

    Uses streamed send_hover_setpoint(0,0,0,z) at 50 Hz (holds absolute
    altitude via ToF, zero body velocity, zero yaw rate).
    """
    global t_start
    z = BOX_Z
    setpoint_hz = 50.0
    ...
    n = max(int(HOVER_S * setpoint_hz), 1)
    print(f'Hover in place for {HOVER_S:.1f}s (hold z={z} m, zero velocity)')
    for _ in range(n):
...
def run_baby_sequence(cf):
    ...
    Rotations are done as RELATIVE yaw steps (two +180 deg for the 360, one +180
    for the half turn) so a full turn doesn't wrap to zero the way an absolute
    yaw=2*pi target would. Position stays fixed during each turn (dx=dy=dz=0).
    """
    global t_start
    z = BOX_Z
    fwd = BABY_FWD
    ...
    # 2) 360 deg turn in place (two relative +180 deg steps; ends back at yaw=0)
    print('360 deg turn in place')
    hlc.go_to(0.0, 0.0, 0.0, math.pi, yaw_dur, relative=True)
    time.sleep(yaw_dur + settle)
    hlc.go_to(0.0, 0.0, 0.0, math.pi, yaw_dur, relative=True)
    time.sleep(yaw_dur + settle)

    # 3) hover
    print(f'Hover for {settle:.1f}s')
    hlc.go_to(fwd, 0.0, z, 0.0, settle)
    time.sleep(settle)

    # 4) 180 deg turn in place -> now facing back toward takeoff (yaw = pi)
    print('180 deg turn in place')
    hlc.go_to(0.0, 0.0, 0.0, math.pi, yaw_dur, relative=True)
    time.sleep(yaw_dur + settle)

    # 5) forward BABY_FWD m back to takeoff (body-forward, holding yaw = pi)
    print(f'Forward {fwd:.1f} m back to takeoff (0, 0)')
    hlc.go_to(0.0, 0.0, z, math.pi, move_dur)
    time.sleep(move_dur + settle)
