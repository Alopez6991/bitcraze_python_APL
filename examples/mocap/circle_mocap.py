"""Mocap-guided circular flight for a Crazyflie.

This example arms the drone, takes off, hovers, moves to the circle start,
flies a circle centered at (0, 0), and lands. The number of circle cycles is
configurable with ``--cycles``.
...
DEFAULT_MOCAP_SYSTEM = 'flapper_02'
...
DEFAULT_HOVER_TIME = 2.0
DEFAULT_TRANSIT_TIME = 5.0
DEFAULT_RADIUS = 2.0
DEFAULT_SECONDS_PER_CYCLE = 16.0
DEFAULT_CYCLES = 1
DEFAULT_YAW_DEG = 0.0
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Take off, hover, and fly a configurable mocap-guided circle.',
    )
    ...
    parser.add_argument('--cycles', type=int, default=DEFAULT_CYCLES,
                        help='Number of full circle cycles to fly, default: %(default)s')
    parser.add_argument('--radius', type=float, default=DEFAULT_RADIUS,
                        help='Circle radius in meters, default: %(default)s')
    parser.add_argument('--height', type=float, default=DEFAULT_TAKEOFF_HEIGHT,
                        help='Flight height in meters, default: %(default)s')
    parser.add_argument('--seconds-per-cycle', type=float, default=DEFAULT_SECONDS_PER_CYCLE,
                        help='Seconds to complete one circle, default: %(default)s')
    parser.add_argument('--hover-seconds', type=float, default=DEFAULT_HOVER_TIME,
                        help='Hover time after takeoff, default: %(default)s')
    parser.add_argument('--transit-seconds', type=float, default=DEFAULT_TRANSIT_TIME,
                        help='Time to move from hover point to circle start, default: %(default)s')
    parser.add_argument('--center-x', type=float, default=0.0,
                        help='Circle center X in meters, default: %(default)s')
    parser.add_argument('--center-y', type=float, default=0.0,
                        help='Circle center Y in meters, default: %(default)s')
    ...
    if args.seconds_per_cycle <= 0.0:
        parser.error('--seconds-per-cycle must be positive')
...
def fly_circle(commander, center_x, center_y, radius, height, yaw_deg, cycles, seconds_per_cycle):
    total_duration = cycles * seconds_per_cycle
    start_time = time.time()
    next_tick = start_time
    ...
    while True:
        ...
        commander.send_position_setpoint(x, y, height, yaw_deg)
...
def run_mission(scf, args):
    ...
    cf.high_level_commander.takeoff(args.height, DEFAULT_TAKEOFF_TIME)
    time.sleep(DEFAULT_TAKEOFF_TIME + 0.5)

    current_x, current_y, _, _ = read_current_state(scf)
    hover_pose = (current_x, current_y, args.height, args.yaw_deg)
    print(f'Hovering for {args.hover_seconds:.1f} s at x={current_x:+.2f} y={current_y:+.2f}')
    stream_position_hold(cf.commander, *hover_pose, duration=args.hover_seconds)

    circle_start = (
        args.center_x + args.radius,
        args.center_y,
        args.height,
        args.yaw_deg,
    )
    print(
        f'Moving to circle start x={circle_start[0]:+.2f} y={circle_start[1]:+.2f} '
        f'over {args.transit_seconds:.1f} s'
    )
    move_line(cf.commander, hover_pose, circle_start, args.transit_seconds)
    stream_position_hold(cf.commander, *circle_start, duration=1.0)

    print(
        f'Flying {args.cycles} circle cycle(s) centered at '
        f'({args.center_x:+.2f}, {args.center_y:+.2f}) with radius {args.radius:.2f} m'
    )
    fly_circle(
        cf.commander,
        args.center_x,
