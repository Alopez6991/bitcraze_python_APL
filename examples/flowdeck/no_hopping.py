...
TARGET_DIST_M = 1.0         # stop after traveling this far [m]
...
# --- Estimator helpers: disable/enable z-range fusion (best-effort) ---


def _set_param_if_exists(scf, name: str, value: str) -> bool:
    try:
...
def try_disable_zrange_in_estimator(scf, record_list: list):
    """Attempt to disable z-range usage in the estimator.
    1) Try a few common parameter names
    2) Fall back to scanning the param TOC for likely kalman.* params containing 'range' and 'use'
    Records any that worked so we can restore later.
    """
    changed = []
    tried = []
    # 1) Known candidates
    known = ['kalman.useZrange', 'kalman.useZRange', 'kalman.useRanging', 'kalman.useRange']
    for pname in known:
        try:
            _ = scf.cf.param.get_value(pname)
        except Exception:
            continue
        tried.append(pname)
        if _set_param_if_exists(scf, pname, '0'):
            changed.append(pname)
            record_list.append(pname)

    # 2) Heuristic scan in TOC
    if not changed:
        try:
            p_toc = scf.cf.param.toc.toc
            for group, params in p_toc.items():
                if 'kalman' not in group.lower():
                    continue
                for name in params.keys():
                    lname = name.lower()
                    if ('range' in lname or 'zrange' in lname or 'tof' in lname or 'height' in lname) and ('use' in lname or lname.startswith('en')):
                        full = f"{group}.{name}"
                        tried.append(full)
                        if _set_param_if_exists(scf, full, '0'):
                            changed.append(full)
                            record_list.append(full)
        except Exception:
            pass

    if changed:
        print(f"[estimator] Disabled z-range via: {', '.join(changed)}")
    else:
        # Help the user by printing a few kalman params that look relevant
        hints = []
        try:
            p_toc = scf.cf.param.toc.toc
            for group, params in p_toc.items():
                ...
                for name in params.keys():
                    lname = name.lower()
                    if ('range' in lname or 'z' in lname) and len(hints) < 12:
                        hints.append(f"{group}.{name}")
        except Exception:
            pass
        print('[estimator] No known z-range toggle found; proceeding with vz=0 control only.')
        if hints:
            print("[estimator] Kalman params with 'range' or 'z' (for reference):")
            for h in hints:
                print(f"  - {h}")


def try_restore_zrange_in_estimator(scf, record_list: list):
    if not record_list:
        return
    restored = []
    for pname in record_list:
        if _set_param_if_exists(scf, pname, '1'):
            restored.append(pname)
    if restored:
        print(f"[estimator] Restored z-range: {', '.join(restored)}")
...
def main():
    ...
    with SyncCrazyflie(URI, cf=cf) as scf:
        ...
        # --- State estimate (x,y,z,yaw) for distance tracking and safe reinit ---
        state = {'x': None, 'y': None, 'z': None, 'yaw': None}
        # Keep a short history of Z to restore pre-spike altitude (avoid hop)
        z_hist = deque(maxlen=10)
        lg_state = LogConfig('state', period_in_ms=100)
        for v in ('stateEstimate.x', 'stateEstimate.y', 'stateEstimate.z', 'stateEstimate.yaw'):
        ...
        def _cb_state(ts, data, lg):
            ...
            if 'stateEstimate.yaw' in data:
                state['yaw'] = float(data['stateEstimate.yaw'])
        ...
        def _reinit_estimator_to_current(pre_spike_z: float | None):
            """Reinitialize the Kalman to current X/Y/Yaw and supplied Z (last good) to avoid hop."""
            try:
                if state['x'] is not None:
                    scf.cf.param.set_value('kalman.initialX', str(state['x']))
                if state['y'] is not None:
                    scf.cf.param.set_value('kalman.initialY', str(state['y']))
                if pre_spike_z is not None:
                    scf.cf.param.set_value('kalman.initialZ', str(pre_spike_z))
                if state.get('yaw') is not None:
                    scf.cf.param.set_value('kalman.initialYaw', str(state['yaw']))
                # Pulse reset
                scf.cf.param.set_value('kalman.resetEstimation', '1')
                time.sleep(0.05)
                scf.cf.param.set_value('kalman.resetEstimation', '0')
                print(
                    f"[estimator] Reinitialized to X={state['x']:.2f}, Y={state['y']:.2f}, Z={pre_spike_z if pre_spike_z is not None else state['z']:.2f}")
            except Exception as e:
                print(f"[estimator] Reinit failed: {e}")
        ...
        def _cb_down(ts, data, lg):
            ...
            if len(hist_r) >= 2:
                ...
                if step_hit and rate_hit and TARGET_DIST_M - state['x'] <= 0.1:
                    Test_var = TARGET_DIST_M - state['x']
                    print(f"Test_var={Test_var}")
                    # Attempt to rewind estimator Z to pre-spike value to avoid hop
                    pre_spike_z = None
                    if len(z_hist) >= 2:
                        pre_spike_z = z_hist[-2]  # one sample before current
                    elif state['z'] is not None:
                        pre_spike_z = state['z']
                    _reinit_estimator_to_current(pre_spike_z)
        ...
        with MotionCommander(scf, default_height=HEIGHT) as mc, Multiranger(scf) as mr:
            ...
            # Immediately disable estimator z-range fusion for the whole trip
            zrange_param_names = []
            try_disable_zrange_in_estimator(scf, zrange_param_names)
            ...
            while True:
                ...
                if stop_now:
                    ...
                    time.sleep(1.5)
                    takeoff_or_landing['on'] = False
                    # Restore estimator settings (optional)
                    try_restore_zrange_in_estimator(scf, zrange_param_names)
                    break
