...
def main():
    ...
    t_epoch = int(time.time())        # for filenames only
    out = Path('logs')
    out.mkdir(exist_ok=True)

    mocap_path = out / f"mocap_{t_epoch}.csv"
    cf_path = out / f"cf_{t_epoch}.csv"
    ...
    # ---- Shared timing: start when mocap first sample arrives ----
    start_evt = Event()
    t0 = {'val': None}  # mutable holder so inner funcs can set/read

    # Protect writers (mocap callback runs in another thread)
    write_lock = Lock()

    # Mocap callback: sets t0 on first sample; logs with perf_counter
    def on_mocap_pose(pose):
        x, y, z, q = pose
        if t0['val'] is None:
            t0['val'] = perf_counter()
            start_evt.set()
        t = perf_counter() - t0['val']
        with write_lock:
            mocap_w.writerow([t, x, y, z, q.w, q.x, q.y, q.z])
            # optional: flush for live monitoring
            # mocap_f.flush()
    ...
    try:
        with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
            ...
            def _cf_cb(timestamp, data, logconf):
                # Wait until mocap has started (t0 set)
